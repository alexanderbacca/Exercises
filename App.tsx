...
const App: React.FC = () => {
  ...
  const [history, setHistory] = useState<HistoryEntry[]>(() => {
    try {
      const raw = localStorage.getItem('ppt_history');
  ...
  const saveData = async () => {
    ...
    // Append one record to the full training history (separate from the 5-item ppt_submissions)
    try {
      const historyEntry: HistoryEntry = {
        date: new Date().toISOString(),
        pulseBefore,
        pulseAfter,
        recoveryPulse: pulseRecovery,
        altitude,
        durationMin: rec.durationMin,
      };
      const rawHistory = localStorage.getItem('ppt_history');
      const existingHistory = rawHistory ? sanitizeHistory(JSON.parse(rawHistory)) : [];
      const updatedHistory = [...existingHistory, historyEntry];
      localStorage.setItem('ppt_history', JSON.stringify(updatedHistory));
      setHistory(updatedHistory);
    } catch (e) {
      console.error('Failed to persist training history:', e);
    }
  ...
  // Compact inline SVG chart of the last 15 sessions, plus quick stats
  const renderProgressChart = () => {
    ...
    const bestSession = validAfter.length > 0
      ? validAfter.reduce((best, current) => (current.pulseAfter < best.pulseAfter ? current : best), validAfter[0])
      : null;
    const bestDate = bestSession
      ? formatDateSafe(bestSession.date, { month: 'short', day: 'numeric', year: '2-digit' })
      : '';

    if (validHistory.length === 0) {
      return (
        <div className="w-full max-w-md mx-auto bg-[#111] rounded-[2rem] p-8 border border-white/10 text-center">
          <p className="text-orange-500 font-black uppercase italic tracking-tight text-lg">
            Your evolution starts with the first session
          </p>
    ...
    const getY = (pulse: number) => {
      const y = padding + chartHeight - ((pulse - minPulse) / pulseRange) * chartHeight;
    ...
    return (
      <div className="w-full max-w-md mx-auto bg-[#111] rounded-[2rem] p-6 border border-white/10">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white font-black italic uppercase tracking-tight text-sm">Pulse Evolution</h3>
        ...
        <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
          ...
          {last15Sessions.map((s, i) => (
            <g key={i}>
              <circle cx={getX(i)} cy={getY(s.pulseBefore)} r="4" fill="#f97316" stroke="#111111" strokeWidth="2" />
              <circle cx={getX(i)} cy={getY(s.pulseAfter)} r="4" fill="#dc2626" stroke="#111111" strokeWidth="2" />
            </g>
          ))}

          {/* X-axis labels (dates) */}
          {last15Sessions.map((s, i) => {
            const step = Math.ceil(last15Sessions.length / 5) || 1;
            if (i % step === 0 || i === last15Sessions.length - 1) {
              const label = formatDateSafe(s.date, { month: 'numeric', day: 'numeric' });
              return (
                <text key={i} x={getX(i)} y={height - 8} textAnchor="middle" fill="#9ca3af" fontSize="10">
                  {label}
                </text>
              );
            }
          ...
          {/* Y-axis labels */}
          {[0, 1, 2, 3].map((i) => {
            const pulseLabel = Math.round(maxPulse - (i / 3) * pulseRange);
            return (
              <text key={i} x={padding - 5} y={padding + (i / 3) * chartHeight + 4} textAnchor="end" fill="#9ca3af" fontSize="10">
                {pulseLabel}
              </text>
            );
          })}
        ...
        <div className="grid grid-cols-2 gap-3 mt-5 pt-5 border-t border-white/10">
          <div className="text-center">
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest">Total Sessions</p>
            <p className="text-white font-black italic text-2xl">{totalSessions}</p>
          </div>
          <div className="text-center">
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest">Best Pulse After</p>
            <p className="text-white font-black italic text-2xl">
              {bestSession ? (
                <>{bestSession.pulseAfter} <span className="text-gray-500 text-sm not-italic font-bold">({bestDate})</span></>
              ) : (
                '\u2014'
              )}
            </p>
